  ...
  <h:dataTable id="perm" value="#{ForumTool.permissions}" var="permission" cellpadding="0" cellspacing="0" >
    <h:column>
      <h:panelGrid id="permissionSet"  columns="2" summary="layout">   
        ...
        <h:panelGroup>
          <h:outputLabel for="viewReadby"><h:outputText value="#{msgs.perm_view_readby}" /></h:outputLabel>
        </h:panelGroup>       
        <h:panelGroup>
          <h:selectOneRadio id="viewReadby" value="#{permission.viewReadby}"  layout="pageDirection"  onclick="setCorrespondingLevel(this.name);"  disabled="#{not ForumTool.editMode}" styleClass="checkbox inlineForm">
                        <f:selectItems   value="#{ForumTool.viewReadbyOptions}" />
                      </h:selectOneRadio>
        </h:panelGroup>   
